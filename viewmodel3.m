function viewmodel(s,s2,formula)
...
if ~isvariable(s,'realspiketimes')
...
if nargin>1 & isstruct(s2)
    display('Using second input to calculate model...')
    [s.mBoth,s.bothShift]=bestFit(s2,0);
%     [s.mLeft,s.mRight,s.leftShift,s.rightShift,s.leftShiftEye,s.rightShiftEye]=bestFit(s2,0);
else
        [s.mBoth,s.bothShift]=bestFit(s,0,formula);
...
function [model,bestShift]= bestFit(r,fitTtype,formula)
if nargin<2
...
d=makeSpikeDataset(r,bestShift,'both');
