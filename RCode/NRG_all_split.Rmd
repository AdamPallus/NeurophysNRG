...
t %>%
  group_by(neuron) %>%
  do(dlead=dynamiclead(p=.,lags=seq(10,150,by=10),fit.eq=fit.eq)) ->
  x
x$dlead<-as.numeric(x$dlead)
tt<-left_join(t,x,by='neuron')

t %>% 
  group_by(neuron,trialnum) %>%
  summarize(dlead=dlead[1]) ->
  l
library(data.table)
t<-as.data.table(t)
#We want to shift each trial by the appropriate amount depending on the neuron chosen
t<-as.data.frame(t[,sdflag:=shift(sdf,dlead[1],type='lag'),by=c('neuron,trialnum')])
saveRDS(t,'allNeuronsSplitLag.RDS')
```
...
t %>% 
  group_by(neuron) %>% 
  do(b=calc.relimp(lm(fit.eq,data=.))) -> 
  rimp
