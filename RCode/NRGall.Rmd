...
dynamiclead<-function(p,lags=seq(10,300,by=10)) {
  ...
  for (i in 1:length(lags)) {
    ...
    rsq[i]<- summary(lm(sdflag~head+hv+ha+eye+ev+ea,data=p))$r.squared
  }
  #return(rsq)
...
for (i in 1:length(n)){
  ...
  l<-dynamiclead(temp,lags=seq(10,150,by=10))
  temp<-mutate(temp,leadtime=l,sdflag=lag(sdf,l))
  newt<-rbind(newt,temp)
}

t<-newt
saveRDS('allNeuronsLag.RDS')
```
